...
class TopItemsLists extends StatelessWidget {
  ...
  const TopItemsLists({super.key, required this.order});
    ...
    void blockUnblockClient(dynamic client) async {
      ...
      if (result.successful == true) {
        ...
        showSnackbar(
          appConfigProvider: appConfigProvider,
          label:
              newList == AccessSettingsList.allowed ||
                  newList == AccessSettingsList.disallowed
      ...
      children: order.asMap().entries.map((item) {
        switch (item.value) {
            ...
            return Column(
              children: [
                TopItemsSection(
                  ...
                  data:
                      statusProvider.serverStatus?.stats.topQueriedDomains ??
                      [],
                  withChart: true,
            ...
            return Column(
              children: [
                TopItemsSection(
                  ...
                  data:
                      statusProvider.serverStatus?.stats.topBlockedDomains ??
                      [],
                  withChart: true,
                  withProgressBar: true,
                  buildValue: (v) => v.toString(),
                ...
                TopItemsSection(
                  ...
                  menuOptions: (v) => [
                    if (clientsProvider.clients?.clientsAllowedBlocked != null)
                      MenuOption(
                        title:
                            clientsProvider.checkClientList(v) ==
                                    AccessSettingsList.allowed ||
                                clientsProvider.checkClientList(v) == null
          ...
          case HomeTopItems.topIps:
            return (statusProvider.serverStatus?.stats.topIps.isNotEmpty ??
                    false)
                ? Column(
                    children: [
                      TopItemsSection(
                        label: 'IPs',
                        type: HomeTopItems.topIps,
                        data: statusProvider.serverStatus?.stats.topIps ?? [],
                        withChart: true,
                        withProgressBar: true,
                        buildValue: (v) => v.toString(),
                        menuOptions: (v) => [
                        ...
                        onTapEntry: (v) =>
                            filterClientLogs(value: v.toString()),
                      ),
                      if (item.key < order.length - 1) ...bottom,
                    ],
                  )
            ...
            return (statusProvider
                ...
                ? Column(
                    children: [
                      TopItemsSection(
                        label: 'Domínios raiz',
                        type: HomeTopItems.rootDomains,
                        data:
                            statusProvider.serverStatus?.stats.topRootDomains ??
                            [],
                        withChart: true,
                        withProgressBar: true,
                        buildValue: (v) => v.toString(),
                        menuOptions: (v) => [
                          MenuOption(
                            ...
                            icon: Icons.block_rounded,
                            action: () => blockUnblock(
                              domain: v.toString().replaceFirst('*.', ''),
                              newStatus: 'block',
                            ),
                          ),
                          MenuOption(
                        ...
                        onTapEntry: (v) => filterDomainLogs(
                          value: v.toString().replaceFirst('*.', ''),
                        ),
                      ),
                      if (item.key < order.length - 1) ...bottom,
                    ],
